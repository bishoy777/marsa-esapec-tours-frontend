    ...
    <div>
        <div class="bg-card border border-border rounded-xl overflow-hidden">
            ...
            <section class="w-full text-center py-4 flex flex-wrap justify-center gap-2" dir="ltr" v-else>
                <button @click="chagePage(pagination?.page - 1)" class="px-3 py-1 border border-border rounded disabled:opacity-40">
                    السابق
                </button>
                <button class="px-3 py-1 rounded border border-border" v-for="(page, index) in totalPages" :key="index"
                    @click="chagePage(page)" :class="{ 'bg-primary': page === pagination?.page }">
                    {{ formatArabicNumber(page) }}
                </button>
                <button class="px-3 py-1 border border-border rounded disabled:opacity-40"
                    @click="chagePage(pagination?.page + 1)">التالي</button>
            </section>
        </div>
