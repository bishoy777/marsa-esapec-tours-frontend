        ...
        <div class="bg-gray-50 py-10">
            <section id="table">
                <UiTableBaseTable :cols="cols" :rows="rows" :loading="pending"><template #edit="{ row }"
                        ...
                        <button class="btn mx-3" @click="removeTaxiReview(row.id.value)">
...
const { data, pending, refresh } = useAsyncData(
    ...
    async () => {
        const { $api } = useNuxtApp()
        return await $api.get('/taxireviews')
    },
    {
        default: () => ({ data: [] }),
...
const submit = async () => {
    ...
    if (!valid) return;
    try {
        const res = await addTaxiReview(formData.value)
...
const removeTaxiReview = async (id: number) => {
    try {
        await deleteTaxiReview(id)
...
const changeStatus = async (param: any) => {
    try {
        await edtiTaxiReviewStatus(param.id, { status: param.status })
